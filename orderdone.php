...
if (isset($_GET['show'])) {
	...
	$sql = "SELECT sum(number_order * price) as total_price,sum(number_order) as total_order,invoice.* FROM invoice inner join invoice_items on id_invoice = id_invoice_fk inner join items on id_item_fk = id_item where states = 1 group by id_invoice order by id_invoice desc";
			...
			<div class="list-item">
				...
				<span>' . time_elapsed_string($row['date']) . ' </span>';
					...
					while ($row = $shows->fetch_assoc()) {
						...
						$i++;
						$all_price = $row['number_order'] * $row['price'];
